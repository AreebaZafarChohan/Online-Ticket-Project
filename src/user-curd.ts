...
import { showPlaces, allClients, Clients} from "./displayTours.js";
import { main } from "./main.js";
import chalk from "chalk";

// This login function is for those pepole who have booked a tour
export async function clientSignIn(): Promise<void> {
    const {clientName, id, password}: {clientName: string;
...
function displayOptions(id: string): void {
    inquirer.prompt([
        {
            type: "list",
            name: "option",
...
function findUserById(id: string): Clients | undefined {
    return allClients.find((client) => client.id === id);
}

// Function to display user's tour information
function displayUserTourInfo(id: string): void {
...
function postponeTour(id: string): void {
    const user = findUserById(id);
    if (!user) {
    ...
    .then((answers: {postponed: boolean}) => {
    ...
    function bookMoreTours(id: string): void {
        const user = findUserById(id);
        if (!user) {
            console.log(chalk.red.bold("User not found!"));
        displayOptions(id);
