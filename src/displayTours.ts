...
export let offerclients: Clients[] = [];

export let otherClients: Clients[] = [];
    ...
    .prompt([
      {
        ...
        name: "menuOption",
        message:
          "Welcome to the JourneyJoy Tour Portal! Please choose an option:\n",
        choices: [
          "Offers",
...
async function trendingDestinations() {
  const {trendingDestination} = await inquirer.prompt([
      ...
      if (selectedDestination) {
        ...
        if (
          currentDateForDestination <= destinationValidity &&
...
async function ForiegnCountriesAndOtherPlaces() {
  const {otherPlaces} = await inquirer.prompt([
      ...
      if (selectedPlace) {
        ...
        if (
          currentDateForPlace <= placeValidity &&
...
async function bookNowOrVisitUs() {
  const {bookNowOrVisitUs}: Answers = await inquirer.prompt([
    {
      type: "list",
...
function generateJourneyJoyId(): string {
...
async function bookNow() {
  ...
  function returnTicket(obj: Clients) {
...
async function getCardDetails() {
  ...
  },
  {
    ...
    validate: (amt: number):  boolean | string | Promise<void> => {
